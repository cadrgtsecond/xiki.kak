...
So you can collapse nested directories.
...
/tmp/xiki-tutorial
  mkdir -p src/
  printf 'echo hello world!' >> src/coolcommand
...
The first part is identical to our example above, just with an arbitrary file instead of `~`.

Then we check if it is a regular file, and use `xdg-open` on it to open it.

Note that in this case we don't really care about children.
